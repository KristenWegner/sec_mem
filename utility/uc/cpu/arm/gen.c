// gen.c - Code generator for ARMv4.
...
#ifdef TCC_ARM_VFP
	RC_FLOAT | RC_F4, // d4/s8
	RC_FLOAT | RC_F5, // d5/s10
	RC_FLOAT | RC_F6, // d6/s12
	RC_FLOAT | RC_F7, // d7/s14
#endif
};
...
static uc_c_type_t float_type, double_type, func_float_type, func_double_type;
#endif
...
const char *default_elfinterp(struct uc_state_t *s)
{
...
{
	int x = *(uint32_t *)(cur_text_section->data + pos);
	x &= 0x00FFFFFF;
...
void load(int r, uc_stack_value_t *sv)
{
	int v, ft, fc, fr, sign;
	uint32_t op;
	uc_stack_value_t v1;
...
#endif
			else o(0xE1A00000 | (intr(r) << 12) | intr(v));
...
void store(int r, uc_stack_value_t *sv)
{
	uc_stack_value_t v1;
	int v, ft, fc, fr, sign;
	uint32_t op;
...
static void gcall_or_jmp(int is_jmp)
{
	...
	if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) 
	{
		...
		if (x) 
		{
			...
			else put_elf_reloc(symtab_section, cur_text_section, ind, R_ARM_PC24, 0);
...
// Determine whether a structure is an homogeneous float aggregate or not. The answer is true if 
// all the elements of the structure are of the same primitive float type and there is less than 
// four elements. Accepts the type corresponding to the structure to be tested.
static int is_hgen_float_aggr(uc_c_type_t *type)
{
...
// Find suitable registers for a VFP Co-Processor Register Candidate (VFP CPRC param) according to 
// the rules described in the procedure call standard for the ARM architecture (AAPCS). If found, 
// the registers are assigned to this VFP CPRC parameter. Registers are allocated in sequence 
// unless a hole exists and the parameter is a single float.
// Param avregs: An opaque structure to keep track of available VFP co-processor regs.
// Param align: alignment constraints for the param, as returned by type_size().
// Param size: size of the parameter, as returned by type_size().
int assign_vfpreg(struct avail_regs *avregs, int align, int size)
{
	...
	else // No special alignment (float or array of float).
	{
		...
		else first_reg = avregs->first_free_reg;
	}
...
// Returns whether all params need to be passed in core registers or not. This is the 
// case for function part of the runtime ABI.
int floats_in_core_regs(uc_stack_value_t *sval)
{
	if (!sval->sym)
		return 0;
...
// Returns the number of registers needed to return the struct, or 0 if returning via 
// struct pointer.
ST_FUNC int gfunc_sret(uc_c_type_t *vt, int variadic, uc_c_type_t *ret, int *ret_align, int *regsize)
{
#ifdef TCC_ARM_EABI
...
// Parameters are classified according to how they are copied to their final destination for the 
// function call. Because the copying is performed class after class according to the order in the 
// union below, it is important that some constraints about the order of the members of this union 
// are respected:
// 
// - CORE_STRUCT_CLASS must come after STACK_CLASS
// - CORE_CLASS must come after STACK_CLASS, CORE_STRUCT_CLASS and VFP_STRUCT_CLASS
...
#define add_param_plan(PL, PP, CL) do { PP.prev = PL->clsplans[CL]; PL->pplans[PL ## _nb] = PP; PL->clsplans[CL] = &PL->pplans[PL ## _nb++]; } while(0)


// Assigns parameters to registers and stack with alignment according to the rules in the procedure 
// call standard for the ARM architecture (AAPCS). The overall assignment is recorded in an array 
// of per parameter structures called parameter plans. The parameter plans are also further 
// organized in a number of linked lists, one per class of parameter (see the comment for the
// definition of union reg_class).
//
// Param nb_args: number of parameters of the function for which a call is generated.
// Param float_abi: float ABI in use for this function call.
// Param plan: the structure where the overall assignment is recorded.
// Param todo: a bitmap that record which core registers hold a parameter.
//
...
static int assign_regs(int nb_args, int float_abi, struct plan *plan, int *todo)
{
	...
	int nsaa; /* Next stacked argument address */
	int plan_nb = 0;
		...
		case VT_LDOUBLE:
			if (float_abi == ARM_HARD_FLOAT) 
			{
				...
				if (is_float(vtop[-i].type.t) || (is_hfa = is_hgen_float_aggr(&vtop[-i].type))) 
				{
					...
					int end_vfpreg = start_vfpreg + ((size - 1) >> 2);
					...
					if (start_vfpreg >= 0) 
					{
						...
						else add_param_plan(plan, pplan, VFP_CLASS);
...
Returns the number of uc_stack_value_t added by this function on the value stack */
...
again:
			...
			case VFP_STRUCT_CLASS:
				...
				if ((pplan->sval->type.t & VT_BTYPE) == VT_STRUCT) 
				{
					...
					if (i == VFP_STRUCT_CLASS) 
					{
						...
						/* No need to write the register used to a uc_stack_value_t since VFP regs
	...
	Maybe redundant after the "lvalue_save" patch in gen.c:gv() */
	...
	* manually, without the help of gv(int). */
	...
	if (todo) 
	{
		...
		for (pplan = plan->clsplans[CORE_STRUCT_CLASS]; pplan; pplan = pplan->prev) 
		{
			...
			/* An uc_stack_value_t can only pin 2 registers at best (r and r2) but a structure
			can occupy more than 2 registers. Thus, we need to push on the value
			stack some fake parameter to have on uc_stack_value_t for each registers used
...
void gfunc_prolog(uc_c_type_t *func_type)
{
	uc_symbol_t *sym, *sym2;
	int n, nf, size, align, rs, struct_ret = 0;
	int addr, pn, sn; /* pn=core, sn=stack */
	uc_c_type_t ret_type;
	...
	n = nf = 0;
	if ((func_vt.t & VT_BTYPE) == VT_STRUCT && !gfunc_sret(&func_vt, func_var, &ret_type, &align, &rs))
	{
		n++;
		struct_ret = 1;
...
#ifdef TCC_ARM_EABI
		if (float_abi == ARM_HARD_FLOAT && !func_var && (is_float(sym2->type.t) || is_hgen_float_aggr(&sym2->type))) 
		{
...
void gen_opi(int op)
{
	...
	case 1:
		...
		{
			uint32_t x = stuff_const(opc | 0x2000000, vtop->c.i);
...
{
	int fneg = 0, r;
	uint32_t x = 0xEE000A00 | T2CPR(vtop->type.t);
...
void gen_opf(int op)
{
	...
	default:
		...
		if (op >= TOK_ULT && op <= TOK_GT) 
		{
			...
			/// Linux FPU emulator doesn't set carry if equal.
