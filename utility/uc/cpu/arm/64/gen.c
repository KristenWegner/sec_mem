...
static uint32_t arm64_movi(int r, uint64_t x)
{
	...
	if ((x & ~(m << 16)) == m)
		return (0x12a00000 | r | (~x >> 11 & 0x1fffe0)); // movn w(r),#(~x >> 16),lsl #16

	if (!~(x | m))
		return (0x92800000 | r | (~x << 5 & 0x1fffe0)); // movn x(r),#(~x)

	if (!~(x | m << 16))
		return (0x92a00000 | r | (~x >> 11 & 0x1fffe0)); // movn x(r),#(~x >> 16),lsl #16

	if (!~(x | m << 32))
		return (0x92c00000 | r | (~x >> 27 & 0x1fffe0)); // movn x(r),#(~x >> 32),lsl #32

	if (!~(x | m << 48))
		return (0x92e00000 | r | (~x >> 43 & 0x1fffe0)); // movn x(r),#(~x >> 32),lsl #32
...
static void arm64_movimm(int r, uint64_t x)
{
	...
	if ((i = arm64_movi(r, x))) // A single mov.
		o(i); 
	else 
	{
		// A movz / movn and 1-3 movks.
		...
		for (i = 0; i < 64; i += 16)
		{
			if (x1 >> i & 0xFFFF)
			{
				o(mov1 | r | (x1 >> i & 0xFFFF) << 5 | i << 17); // movz/movn x(r),#(*),lsl #(i)
				break;
			}
		}
		...
		{
			if (x1 >> i & 0xFFFF)
				o(0xf2800000 | r | (x >> i & 0xFFFF) << 5 | i << 17); // movk x(r),#(*),lsl #(i)
		}
	}
}
...
static void arm64_spoff(int reg, uint64_t off)
{
	...
	if (sub) off = -off;
	if (off < 4096)
		o(0x910003e0 | sub << 30 | reg | off << 10); // (add|sub) x(reg),sp,#(off)
	else 
	{
...
static void arm64_ldrx(int sg, int sz_, int dst, int bas, uint64_t off)
{
	...
	if (!(off & ~((uint32_t)0xfff << sz)))
		o(0x39400000 | dst | bas << 5 | off << (10 - sz) | (uint32_t)!!sg << 23 | sz << 30); // ldr(*) x(dst),[x(bas),#(off)]
	else if (off < 256 || -off <= 256)
		o(0x38400000 | dst | bas << 5 | (off & 511) << 12 | (uint32_t)!!sg << 23 | sz << 30); // ldur(*) x(dst),[x(bas),#(off)]
	else 
	{
		arm64_movimm(30, off); // Use x30 for offset.
		o(0x38206800 | dst | bas << 5 | (uint32_t)30 << 16 | (uint32_t)(!!sg + 1) << 22 | sz << 30); // ldr(*) x(dst),[x(bas),x30]
	}
}
...
static void arm64_ldrv(int sz_, int dst, int bas, uint64_t off)
{
	...
	if (!(off & ~((uint32_t)0xfff << sz)))
		o(0x3d400000 | dst | bas << 5 | off << (10 - sz) | (sz & 4) << 21 | (sz & 3) << 30); // ldr (s|d|q)(dst),[x(bas),#(off)]
	else if (off < 256 || -off <= 256)
		o(0x3c400000 | dst | bas << 5 | (off & 511) << 12 | (sz & 4) << 21 | (sz & 3) << 30); // ldur (s|d|q)(dst),[x(bas),#(off)]
	else 
	{
		arm64_movimm(30, off); // Use x30 for offset.
		o(0x3c606800 | dst | bas << 5 | (uint32_t)30 << 16 | sz << 30 | (sz & 4) << 21); // ldr (s|d|q)(dst),[x(bas),x30]
	}
}
...
static void arm64_ldrs(int reg_, int size)
{
	...
	case 16:
		o(0xa9400000 | reg | (reg + 1) << 10 | reg << 5); // ldp x(reg),x(reg+1),[x(reg)]
		break;
	}
}
...
static void arm64_strx(int sz_, int dst, int bas, uint64_t off)
{
	uint32_t sz = sz_;
	if (!(off & ~((uint32_t)0xFFF << sz)))
		o(0x39000000 | dst | bas << 5 | off << (10 - sz) | sz << 30); // str(*) x(dst),[x(bas],#(off)]
	else if (off < 256 || -off <= 256)
		o(0x38000000 | dst | bas << 5 | (off & 511) << 12 | sz << 30); // stur(*) x(dst),[x(bas],#(off)]
	else 
	{
		arm64_movimm(30, off); // Use x30 for offset.
		o(0x38206800 | dst | bas << 5 | (uint32_t)30 << 16 | sz << 30); // str(*) x(dst),[x(bas),x30]
	}
}
...
static void arm64_strv(int sz_, int dst, int bas, uint64_t off)
{
	...
	if (!(off & ~((uint32_t)0xfff << sz)))
		o(0x3d000000 | dst | bas << 5 | off << (10 - sz) | (sz & 4) << 21 | (sz & 3) << 30); // str (s|d|q)(dst),[x(bas),#(off)]
	else if (off < 256 || -off <= 256)
		o(0x3c000000 | dst | bas << 5 | (off & 511) << 12 | (sz & 4) << 21 | (sz & 3) << 30); // stur (s|d|q)(dst),[x(bas),#(off)]
	else 
	{
		arm64_movimm(30, off); // Use x30 for offset
		o(0x3c206800 | dst | bas << 5 | (uint32_t)30 << 16 | sz << 30 | (sz & 4) << 21); // str (s|d|q)(dst),[x(bas),x30]
	}
}


static void arm64_sym(int r, uc_symbol_t *sym, unsigned long addend)
{
	// Currently the linker does not generate copy relocations for STT_OBJECTs when invoked 
	// with '-run'. This typically results in 'R_AARCH64_ADR_PREL_PG_HI21 relocation failed' 
	// when a program refers to stdin. A workaround is to avoid that relocation and use only 
	// relocations with unlimited range.
...
ST_FUNC void load(int r, uc_stack_value_t *sv)
{
	...
	if (svr == (VT_LOCAL | VT_LVAL)) 
	{
		...
		else arm64_ldrx(!(svtt & VT_UNSIGNED), arm64_type_size(svtt), intr(r), 29, svcul);
	...
	if ((svr & ~VT_VALMASK) == VT_LVAL && svrv < VT_CONST) 
	{
		...
		else arm64_ldrx(!(svtt & VT_UNSIGNED), arm64_type_size(svtt), intr(r), intr(svrv), 0);
	...
	if (svr == (VT_CONST | VT_LVAL | VT_SYM)) 
	{
		...
		else arm64_ldrx(!(svtt & VT_UNSIGNED), arm64_type_size(svtt), intr(r), 30, 0);
	...
	if (svr < VT_CONST) 
	{
		...
		{
			if (svtt == VT_LDOUBLE)
				o(0x4ea01c00 | fltr(r) | fltr(svr) << 5); // mov v(r).16b,v(svr).16b
			else o(0x1e604000 | fltr(r) | fltr(svr) << 5); // fmov d(r),d(svr)
		}
...
ST_FUNC void store(int r, uc_stack_value_t *sv)
{
	int svtt = sv->type.t;
	int svr = sv->r & ~VT_LVAL_TYPE;
	int svrv = svr & VT_VALMASK;
	...
	if (svr == (VT_LOCAL | VT_LVAL)) 
	{
		...
		else arm64_strx(arm64_type_size(svtt), intr(r), 29, svcul);
	...
	if ((svr & ~VT_VALMASK) == VT_LVAL && svrv < VT_CONST) 
	{
		...
		else arm64_strx(arm64_type_size(svtt), intr(r), intr(svrv), 0);
	...
	if (svr == (VT_CONST | VT_LVAL | VT_SYM)) 
	{
		...
		else arm64_strx(arm64_type_size(svtt), intr(r), 30, 0);
...
static void arm64_gen_bl_or_b(int b)
{
	...
	else o(0xd61f0000 | (uint32_t)!b << 21 | intr(gv(RC_R30)) << 5); // br/blr
}


static int arm64_hfa_aux(uc_c_type_t *type, int *fsize, int num)
{
	if (is_float(type->t)) {
...
static int arm64_hfa(uc_c_type_t *type, int *fsize)
{
...
static unsigned long arm64_pcs_aux(int n, uc_c_type_t **type, unsigned long *a)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if ((type[i]->t & VT_ARRAY) || (type[i]->t & VT_BTYPE) == VT_FUNC)
			size = align = 8;
		else size = type_size(type[i], &align);

		if (hfa) ; // B.2.
		else if (size > 16) // B.3: replace with pointer.
		{
			if (nx < 8)
				a[i] = nx++ << 1 | 1;
			else 
		...
		else if ((type[i]->t & VT_BTYPE) == VT_STRUCT) // B.4.
			size = (size + 7) & ~7;

		if (is_float(type[i]->t) && nv < 8) // C.1.
		{
			a[i] = 16 + (nv++ << 1);
		...
		if (hfa || (type[i]->t & VT_BTYPE) == VT_LDOUBLE) // C.4.
		{
			ns = (ns + 7) & ~7;
			ns = (ns + align - 1) & -align;
		}

		if ((type[i]->t & VT_BTYPE) == VT_FLOAT) // C.5.
			size = 8;

		if (hfa || is_float(type[i]->t)) // C.6.
		{
			a[i] = ns;
			ns += size;
		...
		if ((type[i]->t & VT_BTYPE) != VT_STRUCT && size <= 8 && nx < 8) // C.7.
		{
			a[i] = nx++ << 1;
		...
		if ((type[i]->t & VT_BTYPE) != VT_STRUCT && size == 16 && nx < 7) // C.9.
		{
			a[i] = nx << 1;
			nx += 2;
		...
		if ((type[i]->t & VT_BTYPE) == VT_STRUCT && size <= (8 - nx) * 8) // C.10.
		{
			a[i] = nx << 1;
			nx += (size + 7) >> 3;
		...
		if ((type[i]->t & VT_BTYPE) == VT_STRUCT) // C.13.
		{
			a[i] = ns;
			ns += size;
...
static unsigned long arm64_pcs(int n, uc_c_type_t **type, unsigned long *a)
{
	unsigned long stack;
...
ST_FUNC void gfunc_call(int nb_args)
{
	...
	for (i = nb_args; i; i--)
	{
		if (a[i] & 1)
		{
			uc_stack_value_t *arg = &vtop[i - nb_args];
	...
	for (i = nb_args; i; i--) 
	{
		...
		else if (a[i] >= 32) 
		{
			...
			{
				gv(RC_INT);
				arm64_strx(arm64_type_size(vtop[0].type.t), intr(vtop[0].r), 31, a[i] - 32);
			}
		}
	...
	for (i = nb_args; i; i--, vtop--) 
	{
		...
		else if (a[i] < 16) // Struct replaced by pointer in register.
		...
		else if (a[i] < 32) 
		{
			...
			if ((vtop->type.t & VT_BTYPE) == VT_STRUCT) 
			{
				...
				for (j = 0; j < n; j++)
					o(0x3d4003c0 | (sz & 16) << 19 | -(sz & 8) << 27 | (sz & 4) << 29 | (a[i] / 2 - 8 + j) | j << 10); // ldr ([sdq])(*),[x30,#(j * sz)]
			}
			else gv(RC_F(a[i] / 2 - 8));
		}
	}
	...
	if ((return_type->t & VT_BTYPE) == VT_STRUCT) 
	{
		...
		else vswap(); // Return in registers: keep the address for after the call.
	}
...

	{
		...
		if (bt == VT_BYTE || bt == VT_SHORT) // Promote small integers.
			o(0x13001c00 | (bt == VT_SHORT) << 13 | (uint32_t)!!(rt & VT_UNSIGNED) << 30); // [su]xt[bh] w0,w0
		else if (bt == VT_STRUCT && !(a[0] & 1)) 
		{
			...
			else if (a[0] == 16) 
			{
				...
				for (j = 0; j < n; j++)
					o(0x3d000100 | (sz & 16) << 19 | -(sz & 8) << 27 | (sz & 4) << 29 | (a[i] / 2 - 8 + j) | j << 10); // str ([sdq])(*),[x8,#(j * sz)]
			}
		}
	}
...
ST_FUNC void gfunc_prolog(uc_c_type_t *func_type)
{
	...
	for (i = 1, sym = func_type->ref->next; sym; i++, sym = sym->next) 
	{
		...
		if (16 <= a[i] && a[i] < 32 && (sym->type.t & VT_BTYPE) == VT_STRUCT) 
		{
			...
			{
				for (j = 0; j < k; j++)
					o(0x3d0003e0 | -(sz & 8) << 27 | (sz & 4) << 29 | ((a[i] - 16) / 2 + j) | (off / sz + j) << 10); // str ([sdq])(*),[sp,#(j * sz)]
			}
		}
	}
...
ST_FUNC void gen_va_arg(uc_c_type_t *t)
{
	...
	else 
	{
		...
		write32le(cur_text_section->data + b1, 0x5400000d | (ind - b1) << 3); // lab1
		...
		else 
		{
			...
			o(0x4c402bdc | (uint32_t)fsize << 7 | (uint32_t)(hfa == 2) << 15 | (uint32_t)(hfa == 3) << 14); // ld1 {v28.(4s|2d),...},[x30]
			o(0x0d00801c | r1 << 5 | (fsize == 8) << 10 | (uint32_t)(hfa != 2) << 13 | (uint32_t)(hfa != 3) << 21); // st(hfa) {v28.(s|d),...}[0],[x(r1)]
		}
		
		write32le(cur_text_section->data + b2, 0x14000000 | (ind - b2) >> 2); // lab2
	}
}
...
ST_FUNC void gfunc_return(uc_c_type_t *func_type)
{
	...
	case 16:
		if ((func_type->t & VT_BTYPE) == VT_STRUCT) 
		{
			...
			for (j = 0; j < n; j++)
				o(0x3d400000 | (sz & 16) << 19 | -(sz & 8) << 27 | (sz & 4) << 29 | j | j << 10); // ldr ([sdq])(*),[x0,#(j * sz)]
		}
		else gv(RC_FRET);
		break;
...
static int arm64_iconst(uint64_t *val, uc_stack_value_t *sv)
{
...
static int arm64_gen_opic(int op, uint32_t l, int rev, uint64_t val, uint32_t x, uint32_t a)
{
	...
	case TOK_SHR:
	{
		...
		else o(0x13000000 | (op == TOK_SHR) << 30 | l << 31 | l << 22 | x | a << 5 | val << 16 | (n - 1) << 10); // lsr/asr
...
ST_FUNC void gen_cvt_itof(int t)
{
	...
	else 
	{
		...
		o(0x1E220000 | (uint32_t)!s << 16 | (uint32_t)(t != VT_FLOAT) << 22 | fltr(d) | l << 31 | n << 5); // [us]cvtf [sd](d),[wx](n)
	}
}
...
ST_FUNC void gen_cvt_ftoi(int t)
{
	...
	else 
	{
		...
		o(0x1e380000 | (uint32_t)!!(t & VT_UNSIGNED) << 16 | (uint32_t)((t & VT_BTYPE) == VT_LLONG) << 31 | intr(d) | l << 22 | n << 5); // fcvtz[su] [wx](d),[sd](n)
	}
}
...
ST_FUNC void gen_cvt_ftof(int t)
{
	...
	else 
	{
		...
		else o(0x1e624000 | x | a << 5); // fcvt s(x),d(a)
	}
}
