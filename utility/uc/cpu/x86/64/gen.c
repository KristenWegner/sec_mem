// gen.c - Intel/AMD x86-64 code generator.
...
static void gen_modrm_impl(int op_reg, int r, uc_symbol_t *sym, int c, int is_got)
{
...
/* generate a modrm reference. 'op_reg' contains the additional 3
   opcode bits */
static void gen_modrm(int op_reg, int r, uc_symbol_t *sym, int c)
{
...
/* generate a modrm reference. 'op_reg' contains the additional 3
   opcode bits */
static void gen_modrm64(int opcode, int op_reg, int r, uc_symbol_t *sym, int c)
{
	int is_got;
...
void store(int r, uc_stack_value_t *v)
{
	int fr, bt, ft, fc;
	int op64 = 0;
...
{
	uc_symbol_t *sym = external_global_sym(v, &func_old_type, 0);
	oad(0xe8, 0);
...
static int gfunc_arg_size(uc_c_type_t *type) {
	int align;
...
void gfunc_epilog(void)
{
	...
	if (v >= 4096) {
		uc_symbol_t *sym = external_global_sym(TOK___chkstk, &func_old_type, 0);
...
static X86_64_Mode classify_x86_64_inner(uc_c_type_t *ty)
{
	X86_64_Mode mode;
	uc_symbol_t *f;
...
static X86_64_Mode classify_x86_64_arg(uc_c_type_t *ty, uc_c_type_t *ret, int *psize, int *palign, int *reg_count)
{
	X86_64_Mode mode;
