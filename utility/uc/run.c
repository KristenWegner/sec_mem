// run.c - Support for one-step in-memory generation and execution.
...
static int tcc_relocate_ex(uc_state_t *s1, void *ptr, addr_t ptr_diff);

#ifdef _WIN64
static void *win64_add_function_table(uc_state_t *s1);
...
int tcc_relocate(uc_state_t *s1, void *ptr)
{
	int size;
	addr_t ptr_diff = 0;
...
#ifdef HAVE_SELINUX
	{
		...
		if (ptr == MAP_FAILED || prx == MAP_FAILED)
			uc_error("tccrun: could not map memory");
...
#endif
		uc_free(s1->runtime_mem[i]);
#endif
	}
	uc_free(s1->runtime_mem);
}

/* launch the compiled program with the given arguments */
LIBTCCAPI int tcc_run(uc_state_t *s1, int argc, char **argv)
{
...
#define PAGESIZE 4096
	...
	if (mprotect((void *)start, end - start, PROT_READ | PROT_WRITE | PROT_EXEC))
		uc_error("mprotect failed: did you mean to configure --with-selinux?");
#endif
...
static addr_t rt_printline(addr_t wanted_pc, const char *msg)
{
	...
	uc_dbx_symbol_t *stab_sym = NULL, *stab_sym_end, *sym;
	int stab_len = 0;
	char *stab_str = NULL;

	if (stab_section) {
		stab_len = stab_section->data_offset;
		stab_sym = (uc_dbx_symbol_t *)stab_section->data;
	...
	stab_sym_end = (uc_dbx_symbol_t*)((char*)stab_sym + stab_len);
