...
static void print_search_dirs(uc_state_t *s)
{
    printf("install: %s\n", s->tcc_lib_path);
...
static void set_environment(uc_state_t *s)
{
    char * path;
...
static char *default_outputfile(uc_state_t *s, const char *first_file)
{
    char buf[1024];
    char *ext;
    const char *name = "a";
...
redo:
    ...
    for (first_file = NULL, ret = 0;;) {
        struct uc_file_spec_s *f = s->files[s->nb_files - n];
        s->filetype = f->type;
