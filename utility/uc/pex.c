// pex.c - Microsoft Portable Executable (PE) file output.
...
static const char *pe_export_name(uc_state_t *s1, ElfW(Sym) *sym)
{
...
static int pe_find_import(uc_state_t * s1, ElfW(Sym) *sym)
{
    char buffer[200];
    const char *s, *p;
    int sym_index = 0, n = 0;
...
static void pe_align_section(uc_section_t *s, int a)
{
...
/* ------------------------------------------------------------- */
static int pe_section_class(uc_section_t *s)
{
    int type, flags;
    const char *name;
...
#if 0
    for (i = 1; i < pe->s1->nb_sections; ++i) {
        uc_section_t *s = pe->s1->sections[i];
        int type = s->sh_type;
...
{
    uc_section_t *sr = text_section->reloc;
    ElfW_Rel *rel, *rel_end;
...
static void pe_print_section(FILE * f, uc_section_t * s)
{
...
static void pe_print_sections(uc_state_t *s1, const char *fname)
{
    uc_section_t *s;
    FILE *f;
    int i;
...
static int add_dllref(uc_state_t *s1, const char *dllname)
{
    uc_dll_reference_t *dllref;
    int i;
...
static int pe_load_res(uc_state_t *s1, int fd)
{
    struct pe_rsrc_header hdr;
    uc_section_t *rsrc_section;
...
/* ------------------------------------------------------------- */
static int pe_load_def(uc_state_t *s1, int fd)
{
...
/* ------------------------------------------------------------- */
static int pe_load_dll(uc_state_t *s1, const char *filename)
{
    char *p, *q;
    int index, ret;
...
/* ------------------------------------------------------------- */
ST_FUNC int pe_load_file(struct uc_state_s *s1, const char *filename, int fd)
{
    int ret = -1;
    char buf[10];
...
static void pe_add_runtime(uc_state_t *s1, struct pe_info *pe)
{
    const char *start_symbol;
    int pe_type = 0;
    int unicode_entry = 0;
...
static void pe_set_options(uc_state_t * s1, struct pe_info *pe)
{
    if (PE_DLL == pe->type) {
