...
#ifdef CONFIG_TCC_BCHECK /* bound check related sections */
...
ST_DATA uc_section_t *symtab_section; /* symbol sections */
ST_DATA uc_section_t *stab_section, *stabstr_section; /* debug sections */
...
ST_FUNC void tccelf_stab_new(uc_state_t *s)
{
    ...
    stab_section->sh_entsize = sizeof(uc_dbx_symbol_t);
...
static void free_section(uc_section_t *s)
{
    uc_free(s->data);
}


ST_FUNC void tccelf_delete(uc_state_t *s1)
{
    int i;
    ...
    for ( i = 0; i < s1->nb_loaded_dlls; ++i) {
        uc_dll_reference_t *ref = s1->loaded_dlls[i];
        if ( ref->handle )
#ifdef _WIN32
...
ST_FUNC void tccelf_end_file(uc_state_t *s1)
{
    ...
    tr = uc_malloc_z(nb_syms * sizeof *tr);
    ...
    for (i = 1; i < s1->nb_sections; ++i) {
        ...
        if (sr->sh_type == SHT_RELX && sr->link == s) {
            ...
            for (; rel < rel_end; ++rel) {
                ...
                //if (n < 0) uc_error("internal: invalid symbol index in relocation");
...
ST_FUNC uc_section_t *new_section(uc_state_t *s1, const char *name, int sh_type, int sh_flags)
{
    ...
    sec = uc_malloc_z(sizeof(uc_section_t) + strlen(name));
...
ST_FUNC uc_section_t *new_symtab(uc_state_t *s1,
                           const char *symtab_name, int sh_type, int sh_flags,
                           const char *strtab_name,
                           const char *hash_name, int hash_sh_flags)
{
    uc_section_t *symtab, *strtab, *hash;
    int *ptr, nb_buckets;
...
static void rebuild_hash(uc_section_t *s, unsigned int nb_buckets)
{
    ElfW(Sym) *sym;
...
ST_FUNC int put_elf_sym(uc_section_t *s, addr_t value, unsigned long size,
    int info, int other, int shndx, const char *name)
{
    ...
    if (hs) {
        ...
        if (ELFW(ST_BIND)(info) != STB_LOCAL) {
            ...
            nbuckets = base[0];
            h = elf_hash((uint8_t*)s->link->data + name_offset) % nbuckets;
            *ptr = base[2 + h];
            base[2 + h] = sym_index;
            base[1]++;
...
ST_FUNC int find_elf_sym(uc_section_t *s, const char *name)
{
    ...
    h = elf_hash((uint8_t*) name) % nbuckets;
...
void* tcc_get_symbol(uc_state_t *s, const char *name)
{
...
ST_FUNC int set_elf_sym(uc_section_t *s, addr_t value, unsigned long size,
                       int info, int other, int shndx, const char *name)
{
    ElfW(Sym) *esym;
...
/* put relocation */
ST_FUNC void put_elf_reloca(uc_section_t *symtab, uc_section_t *s, unsigned long offset,
                            int type, int symbol, addr_t addend)
{
    ...
    if (!sr) {
        ...
        /* if the symtab is allocated, then we consider the relocation
           are also */
        sr = new_section(this_state, buf, SHT_RELX, symtab->sh_flags);
...
#else
    if (addend)
        uc_error("non-zero addend on REL architecture");
#endif
}

ST_FUNC void put_elf_reloc(uc_section_t *symtab, uc_section_t *s, unsigned long offset,
                           int type, int symbol)
{
...
ST_FUNC void squeeze_multi_relocs(uc_section_t *s, size_t oldrelocoffset)
{
    uc_section_t *sr = s->reloc;
    ElfW_Rel *r, *dest;
    ssize_t a;
    ElfW(Addr) addr;
                      ...
                      unsigned long value)
{
    ...
    sym = section_ptr_add(stab_section, sizeof(uc_dbx_symbol_t));
    if (str) {
...
ST_FUNC void put_stabs_r(const char *str, int type, int other, int desc,
                        unsigned long value, uc_section_t *sec, int sym_index)
{
...
{
    int n;
    struct uc_symbol_attributes_x_s *tab;
    ...
    if (index >= s1->nb_sym_attrs) {
        ...
        while (index >= n)
            n *= 2;
        tab = uc_realloc(s1->sym_attrs, n * sizeof(*s1->sym_attrs));
        s1->sym_attrs = tab;
...
static void sort_syms(uc_state_t *s1, uc_section_t *s)
{
    int *old_to_new_syms;
    ElfW(Sym) *new_syms;
    int nb_syms, i;
    ...
    nb_syms = s->data_offset / sizeof(ElfW(Sym));
    new_syms = uc_malloc(nb_syms * sizeof(ElfW(Sym)));
    old_to_new_syms = uc_malloc(nb_syms * sizeof(int));
...
static void relocate_rel(uc_state_t *s1, uc_section_t *sr)
{
    uc_section_t *s;
    ElfW_Rel *rel;
...
/* count the number of dynamic relocations so that we can reserve
   their space */
static int prepare_dynamic_rel(uc_state_t *s1, uc_section_t *sr)
{
    ElfW_Rel *rel;
...
static void build_got(uc_state_t *s1)
{
...
static struct uc_symbol_attributes_x_s * put_got_entry(uc_state_t *s1, int dyn_reloc_type,
                                       unsigned long size,
                                       int info, int sym_index)
{
    ...
    const char *name;
    ElfW(Sym) *sym;
    struct uc_symbol_attributes_x_s *attr;
    unsigned got_offset;
    char plt_name[100];
    int len;
    ...
    for(i = 1; i < s1->nb_sections; ++i) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel) {
		 ...
		 ELFW(ST_BIND)(sym->st_info) == STB_LOCAL)) {
            ...
            attr = put_got_entry(s1, reloc_type, sym->st_size, sym->st_info, sym_index);
...
/* put dynamic tag */
static void put_dt(uc_section_t *dynamic, int dt, addr_t val)
{
    ElfW(Dyn) *dyn;
...
static void add_init_array_defines(uc_state_t *s1, const char *section_name)
{
    uc_section_t *s;
    long end_offset;
    char sym_start[1024];
...
static int tcc_add_support(uc_state_t *s1, const char *filename)
{
    char buf[1024];
...
ST_FUNC void tcc_add_bcheck(uc_state_t *s1)
{
    ...
    *ptr = 0;
    ...
    if (s1->output_type != TCC_OUTPUT_MEMORY) {
        ...
        uc_section_t *init_section = find_section(s1, ".init");
...
/* add various standard linker symbols (must be done after the
   ...
   symbols)) */
static void tcc_add_linker_symbols(uc_state_t *s1)
{
    char buf[1024];
    int i;
    uc_section_t *s;
    ...
    for(i = 1; i < s1->nb_sections; ++i) {
        ...
        if (s->sh_type == SHT_PROGBITS &&
            ...
            for(;;) {
                ...
                if (!ch)
                    break;
                if (!uc_is_id(ch) && !uc_is_number(ch))
                    goto next_sec;
                p++;
            }
...
static void tcc_output_binary(uc_state_t *s1, FILE *f,
                              const int *sec_order)
{
    uc_section_t *s;
    int i, offset, size;
...
ST_FUNC void fill_got_entry(uc_state_t *s1, ElfW_Rel *rel)
{
    int sym_index = ELFW(R_SYM) (rel->r_info);
    ElfW(Sym) *sym = &((ElfW(Sym) *) symtab_section->data)[sym_index];
    struct uc_symbol_attributes_x_s *attr = get_sym_attr(s1, sym_index, 0);
...
ST_FUNC void fill_got(uc_state_t *s1)
{
    uc_section_t *s;
    ElfW_Rel *rel;
    int i;
...
static void fill_local_got_entries(uc_state_t *s1)
{
    ElfW_Rel *rel;
...
static void bind_exe_dynsyms(uc_state_t *s1)
{
    const char *name;
    int sym_index, index;
    ElfW(Sym) *sym, *esym;
...
static void bind_libs_dynsyms(uc_state_t *s1)
{
    const char *name;
    int sym_index;
    ElfW(Sym) *sym, *esym;
    ...
    for_each_elem(s1->dynsymtab_section, 1, esym, ElfW(Sym)) {
        ...
        if (sym_index && sym->st_shndx != SHN_UNDEF
            ...
            if (ELFW(ST_BIND)(esym->st_info) != STB_WEAK)
                uc_warn("undefined dynamic symbol '%s'", name);
        }
    }
}
...
static void export_global_syms(uc_state_t *s1)
{
    int dynindex, index;
    const char *name;
    ElfW(Sym) *sym;
...
static int alloc_sec_names(uc_state_t *s1, int file_type, uc_section_t *strsec)
{
    int i;
    uc_section_t *s;
    int textrel = 0;
...
static int layout_sections(uc_state_t *s1, ElfW(Phdr) *phdr, int phnum,
                           uc_section_t *interp, uc_section_t* strsec,
...
static void fill_unloadable_phdr(ElfW(Phdr) *phdr, int phnum, uc_section_t *interp,
                                 uc_section_t *dynamic)
{
    ElfW(Phdr) *ph;
...
/* Fill the dynamic section with tags describing the address and size of
   sections */
static void fill_dynamic(uc_state_t *s1, struct dyn_inf *dyninf)
{
    uc_section_t *dynamic = dyninf->dynamic;
...
/* Relocate remaining sections and symbols (that is those not related to
   dynamic linking) */
static int final_sections_reloc(uc_state_t *s1)
{
    int i;
    uc_section_t *s;
...
static void tcc_output_elf(uc_state_t *s1, FILE *f, int phnum, ElfW(Phdr) *phdr,
...
static int tcc_write_elf_file(uc_state_t *s1, const char *filename, int phnum,
...
static void tidy_section_headers(uc_state_t *s1, int *sec_order)
{
    int i, nnew, l, *backmap;
    uc_section_t **snew, *s;
    ElfW(Sym) *sym;

    snew = uc_malloc(s1->nb_sections * sizeof(snew[0]));
    backmap = uc_malloc(s1->nb_sections * sizeof(backmap[0]));
...
static int elf_output_file(uc_state_t *s1, const char *filename)
{
    int i, ret, phnum, shnum, file_type, file_offset, *sec_order;
    struct dyn_inf dyninf = {0};
    ElfW(Phdr) *phdr;
    ElfW(Sym) *sym;
    uc_section_t *strsec, *interp, *dynamic, *dynstr;
    int textrel;
    ...
    if (dynamic) {
        ...
        for(i = 0; i < s1->nb_loaded_dlls; ++i) {
            uc_dll_reference_t *dllref = s1->loaded_dlls[i];
            if (dllref->level == 0)
    ...
    phdr = uc_malloc_z(phnum * sizeof(ElfW(Phdr)));

    /* compute number of sections */
    shnum = s1->nb_sections;

    /* this array is used to reorder sections in the output file */
    sec_order = uc_malloc(sizeof(int) * shnum);
    sec_order[0] = 0;
...
typedef struct SectionMergeInfo {
    uc_section_t *s;            /* corresponding existing section */
...
ST_FUNC int tcc_load_object_file(uc_state_t *s1,
                                int fd, unsigned long file_offset)
{
    ...
    sm_table = uc_malloc_z(sizeof(SectionMergeInfo) * ehdr.e_shnum);
    ...
    if (stab_index && stabstr_index) {
        ...
        a = (uc_dbx_symbol_t *)(s->data + sm_table[stab_index].offset);
        b = (uc_dbx_symbol_t *)(s->data + s->data_offset);
    ...
    /* resolve symbols */
    old_to_new_syms = uc_malloc_z(nb_syms * sizeof(int));
...
static int tcc_load_alacarte(uc_state_t *s1, int fd, int size, int entrysize)
{
...
ST_FUNC int tcc_load_dll(uc_state_t *s1, int fd, const char *filename, int level)
{
    ...
    dllref = uc_malloc_z(sizeof(uc_dll_reference_t) + strlen(soname));
    dllref->level = level;
...
/* return uc_pre_next_expansion ld script token */
static int ld_next(uc_state_t *s1, char *name, int name_size)
{
    int c;
    char *q;
 ...
 redo:
    ...
    case '/':
        minp();
        if (ch == '*') {
            file->buf_ptr = uc_pre_parse_multiline_comment(file->buf_ptr);
            ch = file->buf_ptr[0];
            goto redo;
        } else {
            q = name;
        ...
        break;
    case '\\':
        ch = uc_pre_handle_end_of_buffer();
        if (ch != '\\')
            goto redo;
        /* fall through */
    /* case 'a' ... 'z': */
...
static int ld_add_file(uc_state_t *s1, const char filename[])
{
    if (filename[0] == '/') {
...
static int ld_add_file_list(uc_state_t *s1, const char *cmd, int as_needed)
{
