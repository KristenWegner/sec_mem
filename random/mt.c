...
static uint64_t mt_get(void* state)
{
	int32_t *i = state, j;
	...
	if (*i >= 0x0270)
	{
		for (j = 0; j < 0x0270 - 0x018D; ++j)
		{
			...
			m[j] = m[j + 0x018D] ^ (y >> 1) ^ ((y & 1ULL) ? 0X9908B0DFULL : 0ULL);
		}

		for (; j < 0x0270 - 1; ++j)
		{
			y = (m[j] & 0x80000000UL) | (m[j + 1] & 0x7FFFFFFFUL);
			m[j] = m[j + (0x018D - 0x0270)] ^ (y >> 1) ^ ((y & 1ULL) ? 0X9908B0DFULL : 0ULL);
		}

		y = (m[0x0270 - 1] & 0x80000000UL) | (m[0] & 0x7FFFFFFFUL);
		m[0x0270 - 1] = m[0x018D - 1] ^ (y >> 1) ^ ((y & 1ULL) ? 0X9908B0DFULL : 0ULL);
...
static void mt_seed(void* state, uint64_t seed)
{
	int32_t *i = state, j;
