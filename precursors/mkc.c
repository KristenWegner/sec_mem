...
static uint8_t CRC32FUN[] = { 0x4C, 0x8B, 0xDA, 0xF7, 0xD1, 0x4D, 0x85, 0xC0, 0x74, 0x28, 0x66, 0x0F, 0x1F, 0x44, 0x00, 0x00, 0x45, 0x0F, 0xB6, 0x13, 0x4D, 0x8D, 0x5B, 0x01, 0x8B, 0xC1, 0x8B, 0xD1, 0x4C, 0x33, 0xD0, 0xC1, 0xEA, 0x08, 0x41, 0x0F, 0xB6, 0xC2, 0x41, 0x8B, 0x0C, 0x81, 0x33, 0xCA, 0x49, 0x83, 0xE8, 0x01, 0x75, 0xDE, 0xF7, 0xD1, 0x8B, 0xC1, 0xC3 };
static uint8_t CRC64FUN[] = { 0x4D, 0x85, 0xC0, 0x74, 0x2D, 0x66, 0x66, 0x66, 0x0F, 0x1F, 0x84, 0x00, 0x00, 0x00, 0x00, 0x00, 0x44, 0x0F, 0xB6, 0x12, 0x48, 0x8D, 0x52, 0x01, 0x0F, 0xB6, 0xC1, 0x4C, 0x33, 0xD0, 0x48, 0x8B, 0xC1, 0x48, 0xC1, 0xE8, 0x08, 0x4B, 0x8B, 0x0C, 0xD1, 0x48, 0x33, 0xC8, 0x49, 0x83, 0xE8, 0x01, 0x75, 0xDE, 0x48, 0x8B, 0xC1, 0xC3 };
...
inline static uint32_t rand48_next(void* state)
{
	register uint16_t* s = state;
...
inline static void* mxorpass(void *restrict ptr, register size_t len, uint32_t seed)
{
	...
	while (len-- > 0U) *d++ ^= rand48_next(s);
	s[0] = s[1] = s[2] = 0U;
	return ptr;
}


// Makes a new time-based 64-byte random seed.
...
int main(int argc, char* argv[])
{
	...
	rand48_seed(random_state, (uint32_t)make_seed());
	...
	for (i = 2; i < argc; ++i)
	{
		...
		while (fgets(line_buffer, 0x4000, input))
		{
			...
			else if (strlen(line) > 12 && line[0] == 'D' || line[0] == 'F' || line[0] == 'S')
			{
				...
				/*
				if (kind < 3 && !isalpha(working[0]))
				{
					printf("Error: Invalid \"%s\" for input symbol. Skipping.\n", working);
					continue;
				}
				*/
				...
				entity_xor_key = (rand48_next(random_state) ^ rand48_next(random_state));
				entity_alias_id = (rand48_next(random_state) ^ rand48_next(random_state));
				entity_alias_prefix_index = (rand48_next(random_state) + 1) % sizeof(hex_letter_prefixes);
				entity_op_code = (uint16_t)(rand48_next(random_state) ^ rand48_next(random_state));
				sprintf(entity_alias_name, "%c%08x", hex_letter_prefixes[entity_alias_prefix_index], entity_alias_id);
