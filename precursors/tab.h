// tab.h - Hash table implementation.


#ifndef INCLUDE_TAB_H
#define INCLUDE_TAB_H 1
...
typedef halign(1) struct sm_tab_s
{
...
sm_rc tab_create(sm_t context, sm_tab_t** object, size_t size, sm_hsh64_f hasher);

// Destroys the given hash table in *object. Returns status.
sm_rc tab_destroy(sm_tab_t** object);

// Clears the given hash table without deallocating memory. Returns status.
sm_rc tab_clear(sm_tab_t *restrict object);

// Resizes the specified hash table with the given count of buckets. Returns status.
sm_rc tab_resize(sm_tab_t *restrict object, uint64_t buckets);

// Retrieves an element by key from the given hash table. Result is an iterator to the found element, or tab_iterate_end(object) 
// if the element is absent. Returns status.
sm_rc tab_find(sm_tab_t *restrict object, void* key, sm_tab_iterator_t* result);
